...
MJE 17 Feb 2018
- Modified the script to just produce the input runs for 10% - 50% reduction in fertilizer, to speed up the API runs

MJE 26 April 2018
- Modified the script to just produce data for irrigated systems.
...
if len( sys.argv ) < 1:
    print "\n"
    print "python script carn2o2017_cropland_input_files21.py <number mlra42>"
    print "\n"
...
mariadb_connection = MySQLdb.connect( host='tusker', user='marke', passwd='dough72', db='comet' )
...
total_points_per_file = 100

os.chdir( "/data/paustian/CAR/N2O2017/InputData/InputFiles" )

#Write the SQL query:  Build input datasets by CPS practice
sql = ( "SELECT DISTINCT pccl.id as 'cps_id', pccl.irrigated, pccl.practice, pchmm.historic_management_name, pchmm.modern_management_name " + 
    "FROM car2017.cropland_cps_list pccl JOIN car2017.cropland_historic_and_modern_management pchmm ON pccl.id = pchmm.cps_id " + 
    "WHERE pchmm.mlra = '" + str( mlra42_name ) + "' AND practice in ( '10% fertilizer reduction', '20% fertilizer reduction', '30% fertilizer reduction', '40% fertilizer reduction', '50% fertilizer reduction' ) " +
    "ORDER BY practice;" )
    #"WHERE pchmm.mlra = '" + str( mlra42_name ) + "' AND practice in ( '10% fertilizer increase', '20% fertilizer increase', '10% fertilizer reduction', '20% fertilizer reduction', '30% fertilizer reduction', '40% fertilizer reduction', '50% fertilizer reduction' ) " +
print "sql = " + sql

cursor_practices = mariadb_connection.cursor()
cursor_practices.execute( sql )
cursor_practices_count = cursor_practices.rowcount
print " number_of_practices = " + str( cursor_practices_count )

#see how many points there are to run:
sql = ( "SELECT DISTINCT mcfc.id AS 'id_mcfc', point_x, point_y, lrr, state as 'statecode', county, fips, crop_2010, crop_2011, crop_2012, crop_2013, crop_2014, crop_2015, crop_2016 " +
    "FROM car2017.crops_from_cdl_2010_2016 mcfc JOIN car2017.ceap_region_crops_distinct ceap on mcfc.id = ceap.id_crops_from_cdl_2010_2016 " +
    "WHERE mcfc.id < 235419 AND mcfc.mlra42 = '" + str( mlra42_name ) + "' AND crop_2010 <> 'NoData' AND crop_2011 <> 'NoData' AND crop_2012 <> 'NoData' AND crop_2013 <> 'NoData' AND crop_2014 <> 'NoData' AND crop_2015 <> 'NoData' AND crop_2016 <> 'NoData';" ) 
print "sql = " + sql
...
print "number_of_points = " + str( number_of_points ) + ", number_of_files = " + str( number_of_files )
point_count = 0;
...
for file_counter in range( 0, number_of_files ):
    ...
    if( file_counter <= number_of_files ):
        ...
        sql = ( "SELECT DISTINCT " +
            "mcfc.id AS 'id_mcfc', point_x, point_y, lrr, state as 'statecode', county, fips, crop_2010, crop_2011, crop_2012, crop_2013, crop_2014, crop_2015, crop_2016, " +
            "winter_crop_2009_2010, summer_crop_2010, winter_crop_2010_2011, summer_crop_2011, winter_crop_2011_2012, summer_crop_2012, winter_crop_2012_2013, summer_crop_2013, winter_crop_2013_2014, summer_crop_2014, winter_crop_2014_2015, summer_crop_2015, winter_crop_2015_2016, summer_crop_2016 " +
            "FROM car2017.crops_from_cdl_2010_2016 mcfc JOIN car2017.ceap_region_crops_distinct ceap on mcfc.id = ceap.id_crops_from_cdl_2010_2016 " +
            "WHERE mcfc.id < 235419 AND mlra42 = '" + str( mlra42_name ) + "' AND crop_2010 <> 'NoData' AND crop_2011 <> 'NoData' AND crop_2012 <> 'NoData' AND crop_2013 <> 'NoData' AND crop_2014 <> 'NoData' AND crop_2015 <> 'NoData' AND crop_2016 <> 'NoData' " + 
            "LIMIT " + str( ( file_counter * total_points_per_file ) ) + ", " + str( total_points_per_file ) )
        print "sql = " + sql
        ...
        input_xml_file = "/data/paustian/CAR/N2O2017/InputData/InputFiles/" + input_xml_file_name
        ...
        with open( input_xml_file, 'w' ) as f:
            ...
            for ( cps_id, irrigated, practice, historic_management_name, modern_management_name  ) in cursor_practices:
                ...
                print "   ... File# " + str( file_counter + 1 ) + " of " + str( number_of_files ) + ", practice = [" + practice + "], irrig = [" + irrigated + "], mlra42 = " + str( mlra42_name )

                id_mcfc_string = ''
                
                for ( id_mcfc, point_x, point_y, lrr, statecode, county, fips, crop_2010, crop_2011, crop_2012, crop_2013, crop_2014, crop_2015, crop_2016, winter_crop_2009_2010, summer_crop_2010, winter_crop_2010_2011, summer_crop_2011, winter_crop_2011_2012, summer_crop_2012, winter_crop_2012_2013, summer_crop_2013, winter_crop_2013_2014, summer_crop_2014, winter_crop_2014_2015, summer_crop_2015, winter_crop_2015_2016, summer_crop_2016 ) in cursor_points:
                    ...
                    fileWriteString = fileWriteString +  tab1 + "<Cropland name=\"module:cropland|id:" + str( id_mcfc ) + "|irrigated:" + irrigated + "|mlra42:" + str( mlra42_name ) + "|practice:" + practice + "|crop2010:" + crop_2010 + "|crop2011:" + crop_2011 + "|crop2012:" + crop_2012 + "|crop2013:" + crop_2013 + "|crop2014:" + crop_2014 + "|crop2015:" + crop_2015 + "|crop_2016:" + crop_2016 + "\">\n"
                    ...
                    for current_or_future_management in ( 'current', 'future' ):
                        ...
                        summer_crop_2002 = summer_crop_2016
                        ...
                        winter_crop_2008_2009 = winter_crop_2015_2016
                        ...
                        winter_crop_2019_2020 = winter_crop_2015_2016
                        ...
                        winter_crop_2026_2027 = winter_crop_2015_2016
                        ...
                        for x in range( 0, len( summer_cropList ) ): 
                            ...
                            if( year_str == "2017" or year_str == "2027" ): 
                                ...
                                sql = "SELECT DISTINCT pc.cfarm_cropname1, pc.plant_month1, pc.plant_dayofmonth1, pc.plant_dayofyear1 FROM car2017.cropland_crops pc JOIN car2017.cropland_cps_list pcl ON pc.id_cropland_cps_list = pcl.id WHERE pcl.irrigated = '" + irrigated + "' AND pcl.practice = '" + practice + "' AND pc.current_or_future = '" + current_or_future_management + "' AND pc.statecode = '" + statecode + "' AND pc.cdl_name = '" + next_summer_crop + "' AND sequence = 'summercrop';"
                                #print "\n      ...year = " + str( year )
                                #print "\n      ...sql = " + sql + "\n" 
                            ...
                            if( winter_crop is not None and winter_crop != '' ): 
                                ...
                                sql = "SELECT DISTINCT pc.id as 'id_cropland_crops', pc.statecode, pc.sequence, pc.cdl_name, pc.cropname1, pc.cfarm_cropname1, pc.irrigated1, pc.plant_month1, pc.plant_dayofmonth1, pc.plant_dayofyear1, pc.pltm_or_frst1 FROM car2017.cropland_crops pc JOIN car2017.cropland_cps_list pcl ON pc.id_cropland_cps_list = pcl.id WHERE pcl.irrigated = '" + irrigated + "' AND pcl.practice = '" + practice + "' AND pc.current_or_future = '" + current_or_future_management + "' AND pc.statecode = '" + statecode + "' AND pc.cdl_name = '" + winter_crop + "' AND sequence = 'wintercrop';"
                                #print "\n      ...sql = " + sql + "\n"
                                ...
                                id_cropland_crops2 = data_crop[0]
                            ...
                            if( summer_cropList[x] is not None and summer_cropList[x] != ''):
                                ...
                                if summer_crop is None: summer_crop = ""
                                    
                                if summer_crop == "soy": summer_crop = "soybeans"

                                sql = "SELECT DISTINCT pc.id as 'id_cropland_crops', pc.statecode, pc.sequence, pc.cdl_name, pc.cropname1, pc.cfarm_cropname1, pc.irrigated1, pc.plant_month1, pc.plant_dayofmonth1, pc.plant_dayofyear1, pc.harv_month1, pc.harv_dayofmonth1, pc.harv_dayofyear1, pc.pltm_or_frst1 FROM car2017.cropland_crops pc JOIN car2017.cropland_cps_list pcl ON pc.id_cropland_cps_list = pcl.id WHERE pcl.irrigated = '" + irrigated + "' AND pcl.practice = '" + practice + "' AND pc.current_or_future = '" + current_or_future_management + "' AND pc.statecode = '" + statecode + "' AND pc.cdl_name = '" + str( summer_crop ) + "' and sequence = 'summercrop';" 
                                #print "\n      ...sql = " + sql + "\n"
                                ...
                                id_cropland_crops1 = data_crop[0]
                                ...
                                if( cfarm_cropname1 is not None ):
                                    ...
                                    sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year as 'harv_day_of_year', pch.number_of_harvests, pch.grain, pch.yield as 'yield1', pch.strawstoverhayremoval FROM car2017.cropland_harvests pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pch.id_cropland_crops = " + str( id_cropland_crops1 ) + " AND pc.sequence = 'summercrop' ORDER BY day_of_year;"
                                    #print "\n      ...sql = " + sql + "\n"
                                    ...
                                    sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, pch.tillage_type FROM car2017.cropland_tillage pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pch.id_cropland_crops = " + str( id_cropland_crops1 ) + " AND pc.sequence = 'summercrop' ORDER BY day_of_year;"
                                    #print "\n      ...sql = " + sql + "\n"
                                    ...
                                    sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, pch.nfert_type, pch.nfert_amount, pch.nfert_application_method, pch.nfert_eep FROM car2017.cropland_nfert pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pch.id_cropland_crops = " + str( id_cropland_crops1 ) + " AND pc.sequence = 'summercrop' ORDER BY day_of_year;"
                                    #print "\n      ...sql = " + sql + "\n"
                                    ...
                                    sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, pch.omad_type, pch.omad_amount, pch.omad_percent_n, pch.omad_cn_ratio FROM car2017.cropland_omad pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pc.cdl_name not like '%rice%' AND pch.id_cropland_crops = " + str( id_cropland_crops1 ) + " AND pc.sequence = 'summercrop' ORDER BY day_of_year;"
                                    #print "\n      ...sql = " + sql + "\n"
                                    ...
                                    # loop through Irrigation events
                                        sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, pch.irrig_amount FROM car2017.cropland_irrig pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pch.id_cropland_crops = " + str( id_cropland_crops1 ) + " AND ( pc.sequence = 'summercrop' OR pc.cdl_name in ( 'winter wheat', 'winter oats', 'winter barley' ) ) ORDER BY day_of_year;"
                                        #print "\n      ...sql = " + sql + "\n"
                            ...
                            ( winter_crop != '' and next_summer_crop_class != "perennial" ) \
                                ...
                                fileWriteString = fileWriteString + tab5 + "<TillageList>\n" 
                                # loop through Tillage events
                                sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, tillage_type FROM car2017.cropland_tillage pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pch.id_cropland_crops = " + str( id_cropland_crops2 ) + " AND pc.sequence = 'wintercrop' ORDER BY day_of_year;"
                                #print "\n      ...sql = " + sql + "\n"
                                cursor_tillages = mariadb_connection.cursor()
                                cursor_tillages.execute( sql )
                                ...
                                fileWriteString = fileWriteString + tab5 + "<OMADApplicationList>\n" 
                                # loop through OMAD events
                                sql = "SELECT DISTINCT pch.month, pch.day_of_month, pch.day_of_year, pch.omad_type, pch.omad_amount, pch.omad_percent_n, pch.omad_cn_ratio FROM car2017.cropland_omad pch JOIN car2017.cropland_crops pc ON pch.id_cropland_crops = pc.id WHERE pc.cdl_name not like '%rice%' AND pch.id_cropland_crops = " + str( id_cropland_crops2 ) + " AND pc.sequence = 'wintercrop' ORDER BY day_of_year;"
                                #print "\n      ...sql = " + sql + "\n"
                                cursor_omad = mariadb_connection.cursor()
                                cursor_omad.execute( sql )
